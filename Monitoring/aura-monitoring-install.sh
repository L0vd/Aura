...
echo ''
echo -e 'INSTALLING AURA NODE MONITORING'
...
#Aura monitoring variables template 
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/aurad or /home/user/go/bin/aurad
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: auravaloper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
[[outputs.influxdb]]
  database = "aurametricsdb"
  urls = [ "http://95.216.2.219:8086" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/aura-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
sudo mv telegraf_aura.conf /etc/telegraf/telegraf_aura.conf
...
echo ''
if [[ `sudo systemctl status telegraf_aura | grep active` =~ "running" ]]; then
...
echo ''
echo -e '\e[7mYour Aura node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_aura -f\e[0m'
