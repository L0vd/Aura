...
# Aura node monitoring tool
## Community dashboard by L0vd.com: [Dashboard link](http://monitoring.l0vd.com/d/Aura/aura-monitoring-by-l0vd?orgId=1&refresh=30s)
...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/Aura/main/Monitoring/aura-monitoring-install.sh)
```
...
# make the telegraf user sudo and adm to be able to execute scripts as aura user
...
```
cd $HOME
mkdir aura-monitoring
cd aura-monitoring
wget https://raw.githubusercontent.com/L0vd/Aura/main/Monitoring/monitor.sh
wget https://raw.githubusercontent.com/L0vd/Aura/main/Monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/Aura/main/Monitoring/variables.sh
nano variables.sh
```

Insert your parameters to **variables.sh**:
* full path to aura binary to COS_BIN_NAME ( check ```which aurad```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_aura_node_config/config/config.toml```)
* node validator address to COS_VALOPER ( like ```auravaloper********```)
...
Create telegraf service for aura-node monitoring
```
cat > /etc/systemd/system/telegraf_aura.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_aura.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf.conf /etc/telegraf/telegraf_aura.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_aura
sudo systemctl restart telegraf_aura
```
...
## [Dashboard link](http://monitoring.l0vd.com/d/Aura/aura-monitoring-by-l0vd?orgId=1&refresh=30s)
